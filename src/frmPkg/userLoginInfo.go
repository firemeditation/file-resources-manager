...
func NewIsLoginInfo(id uint32, name string, groupid uint16, groupname string, unitid uint16, unitname string, lastTime time.Time) *IsLoginInfo{
	return &IsLoginInfo{IsLoginInfoBasic {id, name, groupid, groupname, unitid, unitname, UserPower{} }, lastTime}
}
...
func (uil *UserIsLogin) Add (ckcode string, id uint32, name string, groupid uint16, groupname string, unitid uint16, unitname string, lastTime time.Time) (ili *IsLoginInfo, err error) {
	uil.lock.Lock()
	defer uil.lock.Unlock()
	_, found := uil.islogin[ckcode]
	if  found == false {
		uil.islogin[ckcode] = NewIsLoginInfo(id, name, groupid, groupname, unitid, unitname, lastTime)
...
func NewSelfLoginInfo (id uint32, name string, groupid uint16 , groupname string, unitid uint16, unitname string, sid string) *SelfLoginInfo{
	return &SelfLoginInfo{IsLoginInfoBasic {id, name, groupid, groupname, unitid, unitname, UserPower{}}, sid}
}
